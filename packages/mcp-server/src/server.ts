...
import { ClientOptions } from 'sa-test-1';
...
export { ClientOptions } from 'sa-test-1';
...
export function initMcpServer(params: {
  ...
  const client = new SaTest(params.clientOptions);
  const capabilities = {
    ...defaultClientCapabilities,
...
export function init(params: {
  ...
  const client = params.client || new SaTest({ defaultHeaders: { 'X-Stainless-MCP': 'true' } });
